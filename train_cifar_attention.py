...
parser.add_argument("--attention_output", default="all", type=str, help="How to merge attention at different levels")
    ...
    def eval():
        ...
        for images, labels in test_loader:
            ...
            if opt.attention_depth > 0:
                ...
                output = F.log_softmax(model(images))
            loss = F.nll_loss(output, labels)
            val_loss_meter.update(loss.data[0], output.size(0))
            preds = output.max(1)[1]
            val_accuracy_meter.update((preds == labels).float().sum().data[0], output.size(0))
