...
class WideResNetAttention(torch.nn.Module):
    def __init__(self, depth, width, num_classes,  attention_depth, attention_width, reg_w=0, attention_output="all", attention_type="softmax"):
    ...
    def forward(self, x):
        ...
        outputs = []
        for i in range(3):
            group = self.__getattr__("group_%d" %i)
            x = group(x)
            if i == 2:
                x = F.relu(self.bn(x))
                outputs.append(self.__getattr__("att%d" %i)(x))
            elif i in self.attention_layers:
                bn = self.__getattr__("pre_bn%d" %i)
                outputs.append(self.__getattr__("att%d" %i)(F.relu(bn(x))))

        o = F.avg_pool2d(x, 8, 1, 0)
        o = o.view(o.size(0), -1)
